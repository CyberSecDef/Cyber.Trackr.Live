...
/**
 * Pulls the DISA cyber.mil catalog and downloads any STIG bundles
 * we don't already have. Extracts the XCCDF XML into
 * resources/data/stig/ and parks the original ZIP under
 ...
 */
#[AsCommand(
    name: 'app:disa:sync-stig',
    description: 'Sync new STIG bundles from the DISA cyber.mil catalog into the local archive.'
)]
class DisaSyncStigCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Syncing STIGs from DISA' . ($dryRun ? ' (dry-run)' : ''));

        $start = microtime(true);
        try {
            $stats = $this->syncer->sync(
                DisaCatalogSyncer::KIND_STIG,
