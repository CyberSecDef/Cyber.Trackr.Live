...
| `cnssi-1253-controls-raw.json` | (sidecar) | Full parsed control map: title + per-axis booleans + privacy. Source-of-truth for the four `_profile.json` files; regenerated by `tmp/build_cnssi_oscal.py` if the source PDF is updated. |
...
Not included (would require additional manual work or CAC-protected access):

- **CNSSI 1253 overlays** (Classified System Overlay, Space Platform Overlay,
  Privacy Overlay) — these are *additive tailorings* on top of the baseline,
  not standalone baselines. They live in `cnss1253/classified.pdf`,
  `cnss1253/space.pdf`, etc. but aren't yet parsed.
- **Cross Domain Solution Overlay** — FOUO, requires CAC/PIV to download from CNSS.
