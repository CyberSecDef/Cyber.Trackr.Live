...
7. **CISA KEV** — refreshed automatically on every `bin/ship.sh`. To pull manually, `bin/console app:kev:refresh`. The `/vulnerabilities/iavm` page is regenerated from the corpus by `app:vulns:rebuild-toc` (also called from `ship.sh`).
